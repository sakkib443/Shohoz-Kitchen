...
const AuthService = {
    ...
    async loginAs(targetUserId: string, callerRole: string): Promise<{ user: IAuthResponse['user']; tokens: { accessToken: string } }> {
        ...
        // Prevent privilege escalation via impersonation: a super admin can never be
        // impersonated, and only a super admin may impersonate an admin. Regular admins
        // may impersonate customers only.
        if (user.role === 'superadmin') {
            throw new AppError(403, 'A super admin account cannot be impersonated.');
        }
        if (user.role === 'admin' && callerRole !== 'superadmin') {
            throw new AppError(403, 'Only a super admin can impersonate an admin account.');
        }
